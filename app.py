...
# --- ESTRUTURAS DE DADOS (SCHEMA PARA A IA) ---
...
def plan_to_markdown(p: PlanoGerado) -> str:
    cab = "\n".join([f"**{k}:** {v}" for k, v in p.cabecalho.items()])
    rotina_md = "\n".join([f"- **{r.inicio}–{r.fim} | {r.titulo}:** {r.descricao}" for r in p.rotina])
    objetivos_md = "\n".join([f"- {x}" for x in p.objetivos])
    campos_md = "\n".join([f"- {x}" for x in p.campos_experiencia])
    materiais_md = "\n".join([f"- {x}" for x in p.materiais])
    avaliacao_md = "\n".join([f"- {x}" for x in p.avaliacao])
    adapt_md = "\n".join([f"- {x}" for x in p.adaptacoes])
...
with col2:
    ...
    if st.button("✨ Gerar plano com IA", use_container_width=True, type="primary"):
        ...
        with st.spinner("Gerando com IA... Isso pode levar até 2 minutos."):
            try:
...
Retorne um JSON estritamente no formato solicitado. Preencha todos os campos de forma rica e detalhada, com foco na mediação do professor e na observação das crianças.
"""
